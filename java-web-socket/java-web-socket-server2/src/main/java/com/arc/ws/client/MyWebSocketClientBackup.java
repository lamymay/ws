...
// use wss
public class MyWebSocketClientBackup extends WebSocketClient {

    public MyWebSocketClientBackup(URI serverUri) {
        super(serverUri);
    }
...
//总之，"TLS" 是更现代和更安全的协议，推荐在安全通信中使用。通常，你应该使用 "TLS" 来创建 SSLContext 实例，除非你特定需要与旧的 SSL 协议版本进行通信，这种情况下你可以使用 "SSL"。在现代应用程序中，强烈建议使用 "TLS" 来确保通信的安全性和兼容性。
    ...
    public static void main(String[] args) throws Exception {
        ...
        MyWebSocketClientBackup client = new MyWebSocketClientBackup(URI.create(serverUri));

        // 在 WebSocket 客户端连接之前禁用主题备用名称检查
        client.setSocketFactory(new SSLSocketFactory() {
            @Override
            ...
            public Socket createSocket(String host, int port) throws IOException {
                SSLSocket socket = (SSLSocket) getDefault().createSocket(host, port);
